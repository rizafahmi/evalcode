...
You are a worker agent building milestone 3 of Alur. The parent orchestrator is not a human. Stay in agent mode. Do not call `SwitchMode`. Do not use `AskQuestion`. Do not wait for confirmation. Build immediately.
...
1. Implement **only** milestone 3 as defined in the PRD (“Deals”). Do not build later milestones.
...
3. Verify against the “Done when” criteria for milestone 3 in the PRD. `mix test` is required. Use HTTP/browser checks when the feature is visible.
4. Run the project formatter and linter before calling this done.
5. When complete, write `_build_plan/milestones/3-deals/milestone-log.md` with:
