...
You are a worker agent building milestone 4 of Alur. The parent orchestrator is not a human. Stay in agent mode. Do not call `SwitchMode`. Do not use `AskQuestion`. Do not wait for confirmation. Build immediately.
...
1. Implement **only** milestone 4 as defined in the PRD (“Kanban pipeline”). Do not build later milestones.
...
3. Verify against the “Done when” criteria for milestone 4 in the PRD. `mix test` is required. Use HTTP/browser checks when the feature is visible.
4. Run the project formatter and linter before calling this done.
5. When complete, write `_build_plan/milestones/4-kanban-pipeline/milestone-log.md` with:
